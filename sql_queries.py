...
songplay_table_create = ("""CREATE TABLE songplays(songplay_id serial PRIMARY KEY, \
                            start_time timestamp not null, user_id varchar not null, level varchar, song_id varchar, \
...
song_table_create = ("""CREATE TABLE songs(song_id varchar PRIMARY KEY, \
                        title varchar not null, artist_id varchar not null, year int, duration float);""")
...
song_select = ("""SELECT songs.song_id AS songid, songs.artist_id AS artistid
FROM songs 
INNER JOIN artists
